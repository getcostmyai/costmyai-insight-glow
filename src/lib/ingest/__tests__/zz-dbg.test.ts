...
describe("g", () => { it("dbg", async () => {
  ...
  const r = await handleProxy(new Request("http://localhost/v1beta/models/gemini-2.0-flash:generateContent",{method:"POST",headers:{"content-type":"application/json","x-goog-api-key":process.env["GEMINI_API_KEY"]!},body:JSON.stringify({contents:[{parts:[{text:"say ok"}]}],generationConfig:{maxOutputTokens:16}})}),{config,queue});
