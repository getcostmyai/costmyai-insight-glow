 ...
 */
export const TASK_HINTS = ["generation", "code", "classification", "unknown"];
export const UNKNOWN_TASK_HINT = "unknown";
/**
 ...
 */
export const PARSE_STATUSES = ["parsed", "tokens_only", "unparsed"];
/**
...
for (const [provider, hosts] of Object.entries(PROVIDER_HOSTS)) {
    for (const host of hosts)
        HOST_TO_PROVIDER.set(host, provider);
}
...
export function providerForHost(host) {
    ...
    for (const fragment of fragments) {
        if (normalised.includes(fragment))
            return HOST_TO_PROVIDER.get(fragment);
    }
    return normalised;
}
/** The one key a capture is deduplicated on, everywhere. */
export function captureIdempotencyKey(provider, periodStart, periodEnd) {
