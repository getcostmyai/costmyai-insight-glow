...
function Team({ orgId, orgName, manager }: { orgId: string; orgName: string; manager: boolean }) {
  ...
  const invite = useMutation({
    ...
    onError: (e: unknown) => setError(e instanceof Error ? e.message : "Could not send that invite."),
  });
    ...
    <main className="mx-auto min-h-screen w-full max-w-3xl px-6 py-14">
      ...
      <section className="mt-8">
        ...
        <ul className="mt-3 divide-y divide-border overflow-hidden rounded-2xl border border-border bg-card">
          ...
          {members.isPending ? <li className="px-5 py-3.5 text-sm text-muted-foreground">Loading…</li> : null}
        </ul>
      </section>
